...
<div class="container d-flex justify-content-center">
    <form class="col-7" action="{{ url('clientes/editar/'.$data->id) }}" method="post" id="formulario">
        @csrf
        @method('PUT')
        <div class="my-3">
            <!-- <label for="nombre" class="form-label">Nombre de cliente</label> -->
            <input type="text" class="form-control" name="nombre" id="nombre" placeholder="Nombre de cliente" autofocus value="{{ $data->nombre }}">
            <span class="error"></span>
        </div>
        <div class="d-flex">
            <div class="my-3 col-6 pe-3">
                <!-- <label for="cedula" class="form-label">Documento de identidad</label> -->
                <input type="text" class="form-control" name="cedula" id="cedula" placeholder="Documento de identidad" value="{{ $data->cedula }}">
                <span class="error"></span>
            </div>
            <div class="my-3 col-6 ps-3">
                ...
                @else
                <!-- <label for="telefono" class="form-label">Télefono</label> -->
                <input type="text" class="form-control" name="telefono" id="telefono" placeholder="Télefono" value="{{ $data->telefono }}">
                @endif
                <span class="error"></span>
            </div>
        </div>
        <div class="my-3 form-floating">
            ...
            @else
            <textarea class="form-control" name="direccion" placeholder="Dirección" id="direccion" style="height: 100px">{{ $data->direccion }}</textarea>
            @endif
